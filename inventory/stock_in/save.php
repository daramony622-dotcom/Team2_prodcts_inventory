...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($product_id <= 0 || $quantity <= 0 || $purchase_price < 0 || $stock_in_date === false) {
        header('Location: ' . APP_BASE_URL . '/stock_in/stock_in_add.php?error=Invalid input data provided.');
        exit();
    }
    ...
    try {
        ...
        if (!$productStmt->fetch(PDO::FETCH_ASSOC)) {
            header('Location: ' . APP_BASE_URL . '/stock_in/stock_in_add.php?error=Product not found.');
            exit();
        }
        ...
        $pdo->commit();
        header('Location: ' . APP_BASE_URL . '/stock_in/index.php?success=Stock successfully added and product quantity updated.');
        exit();
    } catch (Exception $e) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
        }
        header('Location: ' . APP_BASE_URL . '/stock_in/stock_in_add.php?error=' . urlencode($e->getMessage()));
        exit();
    }
} else {
    header('Location: ' . APP_BASE_URL . '/stock_in/index.php');
    exit();
}