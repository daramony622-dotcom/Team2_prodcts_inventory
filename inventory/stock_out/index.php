...
<section class="space-y-6">
    <div class="bg-gradient-to-r from-slate-900 via-slate-950 to-sky-700 text-slate-100 rounded-3xl px-6 py-8 shadow-[0_35px_90px_-40px_rgba(56,189,248,0.22)]">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl md:text-3xl font-extrabold mb-1">Stock Out Management</h1>
                <p class="text-slate-300 text-sm">Track sales and outgoing inventory records.</p>
            </div>
            <a href="add.php"
                class="inline-flex items-center justify-center bg-sky-500 text-white font-semibold px-4 py-2.5 rounded-2xl shadow-lg shadow-sky-500/20 hover:bg-sky-600 transition">
    ...
    <div class="bg-slate-900/85 rounded-[2rem] shadow-[0_35px_90px_-45px_rgba(56,189,248,0.25)] border border-slate-800/70 overflow-hidden">
        <div class="p-5 border-b border-slate-800 flex items-center justify-between gap-3 flex-wrap">
            <div>
                <h2 class="text-lg font-bold text-slate-100">All Stock Out Records</h2>
                <p class="text-xs text-slate-400">Outgoing inventory log</p>
            </div>
        ...
        <div class="overflow-x-auto p-4 md:p-5 bg-slate-950/80 rounded-b-[2rem]">
            <table class="w-full text-left border-separate border-spacing-y-2">
                ...
                <tbody class="text-sm text-slate-200">
                    ...
                    <tr class="align-top">
                        <td class="py-4 px-4 font-medium text-slate-100 bg-slate-900 rounded-l-xl shadow-sm">
                            #<?= (int) $row['id']; ?></td>
                        ...
                        <td class="py-4 px-4 bg-slate-900 shadow-sm">
                            <span
                                class="inline-flex items-center rounded-full bg-sky-100 text-sky-900 px-3 py-1 text-xs font-bold">-<?= (int) $row['quantity']; ?></span>
                        </td>
                        <td class="py-4 px-4 bg-slate-900 shadow-sm">
                            $<?= number_format((float) $row['selling_price'], 2); ?></td>
                        <td class="py-4 px-4 bg-slate-900 shadow-sm"><?= htmlspecialchars($row['stock_out_date']); ?></td>
                        <td class="py-4 px-4 bg-slate-900 shadow-sm"><?= htmlspecialchars($row['username'] ?? 'System'); ?>
                        </td>
                        <td class="py-4 px-4 text-center bg-slate-900 rounded-r-xl shadow-sm">
                            <a href="delete.php?id=<?= (int) $row['id']; ?>"
                                class="bg-sky-600 hover:bg-sky-700 text-white px-3 py-1.5 rounded text-xs font-semibold"
