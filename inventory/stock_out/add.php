...
<section class="space-y-6">
    <div
        class="bg-gradient-to-r from-slate-900 via-slate-950 to-sky-700 text-slate-100 rounded-3xl px-6 py-8 shadow-[0_35px_70px_-30px_rgba(56,189,248,0.22)]">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl md:text-3xl font-extrabold mb-1">Add Stock Out</h1>
                <p class="text-slate-300 text-sm">Register outgoing stock for a sold or consumed item.</p>
            </div>
            <a href="index.php"
    ...
    <div
        class="max-w-3xl mx-auto bg-slate-950 rounded-[2rem] shadow-[0_35px_90px_-40px_rgba(56,189,248,0.25)] border border-slate-800 overflow-hidden">
        <div class="p-6 md:p-8">
            <form action="save.php" method="POST" class="space-y-4">
                <div>
                    ...
                    <select name="product_id" id="product_id"
                        ...
                        <option value="<?= (int) $prod['id']; ?>">
                            <?= htmlspecialchars($prod['product_name']); ?> (Available Stock:
                            <?= (int) $prod['quantity']; ?>)
                        </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    <label for="selling_price" class="block mb-1 text-sm font-semibold text-slate-200">Selling Price
                        ($)</label>
                ...
                <div>
                    <label for="stock_out_date" class="block mb-1 text-sm font-semibold text-slate-200">Stock Out
                        Date</label>
                    <input type="datetime-local" name="stock_out_date" id="stock_out_date"
                        class="w-full border border-slate-700 rounded-2xl px-3 py-2.5 bg-slate-900 text-slate-100 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none"
